...
export default function PivotExplorer({
  ...
  return <section className="rounded-xl border border-line bg-surface p-4 sm:p-5" aria-labelledby="comparison-title">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <label className="flex min-w-0 flex-col gap-1 text-xs text-muted"><span className="sr-only">Filas</span><span>Primero separa por</span><select aria-label="Filas" value={rowDim} onChange={(event) => setRowDim(event.target.value)} className="min-h-11 w-full min-w-0 rounded-lg border border-line bg-surface px-3 py-2 text-sm text-strong">{allowlist.dimensions.map((dimension) => <option key={dimension} value={dimension}>{characteristicLabel(dimension)}</option>)}</select></label>
      <label className="flex min-w-0 flex-col gap-1 text-xs text-muted"><span className="sr-only">Columnas</span><span>Y, si quieres, también por</span><select aria-label="Columnas" value={colDim} onChange={(event) => setColDim(event.target.value)} className="min-h-11 w-full min-w-0 rounded-lg border border-line bg-surface px-3 py-2 text-sm text-strong"><option value={NONE}>Sin segunda separación</option>{allowlist.dimensions.filter((dimension) => dimension !== rowDim).map((dimension) => <option key={dimension} value={dimension}>{characteristicLabel(dimension)}</option>)}</select></label>
      <label className="flex min-w-0 flex-col gap-1 text-xs text-muted"><span className="sr-only">Métrica</span><span>Resultado que quieres mirar</span><select aria-label="Métrica" value={metric} onChange={(event) => setMetric(event.target.value)} className="min-h-11 w-full min-w-0 rounded-lg border border-line bg-surface px-3 py-2 text-sm text-strong">
      ...
      <label className="flex min-w-0 flex-col gap-1 text-xs text-muted"><span className="sr-only">Agregación</span><span>Cómo resumirlo</span><select aria-label="Agregación" value={agg} onChange={(event) => setAgg(event.target.value as AggKind)} className="min-h-11 w-full min-w-0 rounded-lg border border-line bg-surface px-3 py-2 text-sm text-strong">{AGG_KINDS.map((item) => <option key={item} value={item}>{AGG_LABEL[item]}</option>)}</select></label>
    </div>

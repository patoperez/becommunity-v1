 ...
 *
 * PORTED, DELIBERATELY UNCHANGED, from `src/lib/categories/normalize.ts` on the
 * production branch `claude/bni-executive-preview-hotfix` (commit `022513e`).
 * Unit 6B.4B2L replaces that feature's STORAGE and its SURFACE canonically; it
 * does not re-derive its measured comparison rules. The evaluation those rules
 * carry — a false-merge rate of 0/12 on a labelled fixture, with the digit and
 * negation guards that took blind acceptance from 16.7% to 0% — is evidence
 * about THIS code, so rewriting it would throw the evidence away with it.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 *
 *   `foldCategoryLabel` (./contract.ts) is the MERGING fold. Case and
 *   whitespace only. It is applied automatically because nobody writing a
 *   questionnaire means «Malos resultados» and «malos resultados» as two
 ...
 *
 *   Everything below is a COMPARISON key. Two labels sharing one is a question
 ...
 * `survey_response` keeps exactly what was imported.
 *
...
/** How two labels came to be compared. Ordered from most to least defensible. */
 ...
 * while «51 a 100» and «61 a 100» never do. Labels that are essentially numbers
...
/**
 * Every comparison key for one raw label, strongest rule first.
 *
 * A caller compares labels by walking these in order and stopping at the first
 * rule under which two agree — so a pair is always attributed to the STRONGEST
 * rule that explains it, never to a weaker one that also would.
 */
export function comparisonKeys(raw: string): { rule: NormalizationRule; key: string }[] {
  const canonical = canonicalText(raw);
  return [
    // `unicode` compares the canonicalised text as written: two labels that
...
/**
 * Whether two labels differ ONLY by invisible characters or exotic spacing —
 ...
 * is precisely the pair this product exists to notice. Truncating to a prefix
 * is not stemming and does not pretend to be — it is a deliberately dumb,
 * inspectable rule whose failure mode is over-generation of QUESTIONS, never a
 * merge.
 *
 ...
 * applied afterwards, by the caller, as a second opinion on short labels.
 */
 ...
 *
 * Labels longer than `MAX_EDIT_LENGTH`, or whose lengths differ by more than
...
/**
 * Whether two labels carry different digits.
 *
...
/**
 * A label that reads as a number, a range or a percentage rather than a phrase.
 *
 * «51% a 100%», «+100%», «2026» and «1 a 5» are numbers with decoration. A
 * label carrying more than three letters is a phrase, whatever digits it also
 * contains.
 */

...
//   executed). No PostgreSQL instance is available in this environment, so the
//   assertions in sections [13] to [16] read migration 0024's text: the grants,
//   the empty search path, the FOR UPDATE lock, the subtransaction, the ledger
//   vocabulary, the rollback ordering against every ON DELETE RESTRICT edge in
//   0022/0023, and the exact reverse script. Executing those statements is
//   `scripts/canonical-commit-live-test.mjs`, which needs a disposable database
//   and has NOT been run. This gate never reports a database-executed result.
//
...
console.log("\n[16] STRUCTURAL — the ownership ledger and the rollback order");
{
  ...
  const ordered = (rollback.match(/ordered_tables text\[\] := array\[([\s\S]*?)\];/)?.[1] ?? "")
    ...
    .map((entry) => entry.trim().replace(/^'|'$/g, ""))
    .filter(Boolean);
  check(ordered.length === 30, `the rollback walks 30 tables in order (${ordered.length})`);
  const covered = new Set([...ordered, "person_private"]);
  ...
  const position = new Map([...ordered, "person_private"].map((table, index) => [table, index]));
  ...
  check(
    ...
    "and the shared person identity is handled last of all",
  );
  check(
...
console.log(
  "NOTE: sections [14] to [17] read migration 0024's SQL. They are STRUCTURAL proof.\n" +
    "      No PostgreSQL statement in this unit has been executed anywhere.\n" +
    "      Database-executed proof is scripts/canonical-commit-live-test.mjs, which\n" +
    "      requires a disposable local database and has NOT been run.",
);

...
| **ADAPT** | 171 | `fingerprint.ts` | `SHA256_HEX`, `STUDY_FINGERPRINT_PATTERN`, `IDEMPOTENCY_KEY_PATTERN` and `definitionHash`. The patterns are mirrored in `PublicationMetadata` so a document that validates locally is one the database will accept. The hashing itself is 6B, because nothing in 6A writes a revision. |
